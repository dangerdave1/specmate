...
export class BDDConnection1Tool extends ConnectionToolBase<BDDConnection> {

    protected modelType: { className: string; } = BDDModel;

    public name = 'tools.addBdd1Connection';
    public icon = 'sitemap';

    protected getFactory(e1: IModelNode, e2: IModelNode): ElementFactoryBase<IModelConnection> {
        return new BDDConnectionFactory(true, e1, e2, this.dataService);
    }
}
